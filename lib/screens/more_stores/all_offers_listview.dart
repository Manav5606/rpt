...
class ListViewChild extends StatelessWidget {
  ...
  ListViewChild({Key? key, required this.color, required this.inStoreModel}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        ...
        await _moreStoreController.getStoreData(id: inStoreModel?.id ?? '', businessId: inStoreModel?.businesstype ?? '');
      },
      child: Padding(
          ...
          children: [
            inStoreModel!.logo!.isEmpty
                ? Padding(
                    ...
                    child: CircleAvatar(
                      child: Text(inStoreModel?.name?.substring(0, 1) ?? "", style: TextStyle(fontSize: SizeUtils.horizontalBlockSize * 6)),
                ...
                children: [
                  ((inStoreModel?.premium ?? false) == true)
                      ? Text(
                          ...
                          style: TextStyle(color: AppConst.kPrimaryColor, fontSize: SizeUtils.horizontalBlockSize * 4),
                        )
                      : SizedBox(),
                  Text(
                    inStoreModel?.name ?? '',
                    ...
                    children: [
                      (inStoreModel?.calculateDistance != null)
                          ? Container(
                              ...
                              child: Text("${(inStoreModel!.calculateDistance!.toInt() / 1000).toStringAsFixed(2)} km away",
                                  style: TextStyle(
